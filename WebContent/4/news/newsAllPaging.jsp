...
<c:if test="${pageVO.prev }">
		<li class="page-item"><a class="page-link" href="newsAllPaging.jsp?pageNum=${pageVO.startPage - 1 }&amount=${pageVO.amount}">이전</a></li>
</c:if>
       		
	<!-- 1. 페이지번호 처리 -->
	<c:forEach var="num" begin="${pageVO.startPage }" end="${pageVO.endPage }">
		<li class="${pageVO.pageNum eq num ? 'disabled ' : '' }page-item">
		<a class="page-link" href="newsAllPaging.jsp?pageNum=${num }&amount=${pageVO.amount}">${num }</a></li>
	</c:forEach>
	
	<!-- 3. 다음버튼 활성화 여부 -->
	<c:if test="${pageVO.next }">
		<li class="page-item"><a class="page-link" href="newsAllPaging.jsp?pageNum=${pageVO.endPage + 1 }&amount=${pageVO.amount}">다음</a></li>
	</c:if>
  </ul>
</nav>
...
function change(a){
	location.href="newsAllPaging.jsp?pageNum=1&amount=" + a.value;
}
</script>
